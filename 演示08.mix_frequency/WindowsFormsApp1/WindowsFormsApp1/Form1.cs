...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        double nh;
        double k1; // Wave number (2 * pi / lambda)
        double k2;
        ...
        {
            InitializeComponent();
            lambda1 = Convert.ToDouble(speed) / Convert.ToDouble(freq1); // Calculate the wavelength based on the speed of sound and frequency
            //Console.WriteLine($"Lambda = {lambda1} m"); // Print the wavelength to the console for debugging purposes
            lambda2 = Convert.ToDouble(speed) / Convert.ToDouble(freq2);
            k1 = 2 * Math.PI / lambda1; // Wave number
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            wt1 = 2.0 * Math.PI * freq1 * dt / 20000.0;
            wt2 = 2.0 * Math.PI * freq2 * dt / 20000.0;
            ...
            {
                ys1[i] = Math.Cos(k1*r1 + ts[i] - wt1) + 0.2 *Math.Cos(k2 * r1 + 3 *ts[i] - wt2);
                ys2[i] = Math.Cos(k1*r2 + ts[i] - wt1 + 2.0*Math.PI/3.0) + 0.2 *Math.Cos(k2 * r2 + 3 *ts[i] - wt2 + 2.0 * Math.PI / 3.0);
                ys3[i] = Math.Cos(k1*r3 + ts[i] - wt1 + 4.0 * Math.PI/3.0 + eps) + 0.2 *Math.Cos(k2 * r3 + 3 *ts[i] - wt2 + 4.0 * Math.PI / 3.0 + eps);
                ysTotal[i] = ys1[i] + ys2[i] + ys3[i];

                ys21[i] = Math.Cos(k1 * r21 + ts[i] - wt1) + 0.2 *Math.Cos(k2 * r21 + 3 *ts[i] - wt2);
                ys22[i] = Math.Cos(k1 * r22 + ts[i] - wt1 + Math.PI + eps) + 0.2 * Math.Cos(k2 * r22 + 3 *ts[i] - wt2 + Math.PI + eps);
